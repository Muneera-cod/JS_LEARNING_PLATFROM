    ...
    { subview === 0 &&
      ...
      <div className='flex flex-col gap-[0.5rem] ' >
        <div className='flex justify-start items-center'>
         ...
         {filteredData && filteredData.length ? ([...filteredData].sort((a,b)=> a.order - b.order ).map((x)=><QuestionContainerLearner  key={x.id}  id={x.id}  question={x.title} level={x.level} category={x.category}  />)):<div className='min-h-[32rem] flex items-center justify-center gap-2'><p>No {`${selectedLevel ? selectedLevel : null}`} Challenges added</p><IconMoodEmpty className='w-[20px] h-[20px]'/></div>}
       </div>
       </> }
    { subview === 1 && <Outlet/> }
    </>
