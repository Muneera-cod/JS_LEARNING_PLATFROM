  ...
  return (
    ...
    <IconMenu2 className='sm:hidden md:block ml-6'  onClick={()=>setOpen(!open)}/>
     <div className='flex sm:flex-row md:flex-col   md:py-40  w-full'>
       {Data.map((x,index)=>{
         return(
       <div onClick={()=>{handleNavigation(x.link,index)}} key={x.id} className='cursor-pointer flex  sm:flex-col md:flex-row md:gap-[1rem] items-center flex-1 hover:bg-textCLr hover:bg-opacity-20 sm:px-2 md:px-6 sm:py-2 md:py-4'>
       {x.icon}
